...
- **Tool Call JSON 防护**：模型返回的工具参数会先校验 JSON；非法参数不会写入会话历史，旧会话中的坏 tool_call 组也会在发送给模型前过滤
...
容器启动时会短暂以 root 执行 `docker-entrypoint.sh`，创建并修正 `/app/sessions`、`/app/workspace/.tmp`、`skills`、`wiki`、`transcripts` 等运行时目录的 owner，然后通过 `gosu` 降权为 `APP_UID:APP_GID` 运行 `python server.py`。这样 Agent 不会长期以 root 运行，生成的会话、技能、wiki 文件也能被宿主机用户正常编辑。

Linux 宿主机建议在 `.env` 中设置：
...
| `APP_UID` | Docker entrypoint 修正 bind mount 权限并降权运行时使用的宿主机 UID |
| `APP_GID` | Docker entrypoint 修正 bind mount 权限并降权运行时使用的宿主机 GID |
...
`tests/test_state_flow.py`、`tests/test_subagent_stability.py` 和 `tests/test_tool_call_json_guard.py` 覆盖上下文压缩不变量、子 Agent task/tool 运行中事件、父 Agent SSE 转发、刷新恢复、`SUBAGENT_LLM_*` 优先级，以及非法 tool_call 参数不会污染会话历史的防护逻辑。
